...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      from: process.env.EMAIL_FROM || "Nuisible 11 <contact@nuisible11.fr>",
      to: process.env.EMAIL_TO || "contact@nuisible11.fr", 
      replyTo: email,
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; padding: 24px; border-radius: 12px;">
          <h2 style="color: #0056b3; margin-bottom: 20px; font-weight: 900;">🔥 Nouveau Contact - Nuisible 11</h2>
